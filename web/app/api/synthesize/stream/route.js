...
export async function GET(req) {
  ...
  try {
    ...
    const existing = synthesizeSessionStore.getLogs(sessionId);
    for (const line of existing) {
      write(`data: ${JSON.stringify({ type: 'log', line })}\n\n`);
    }
    write(`data: ${JSON.stringify({ type: 'status', status: sess.status })}\n\n`);

    synthesizeSessionStore.addClient(sessionId, writer);
  ...
  const close = () => {
    ...
    try { synthesizeSessionStore.removeClient(sessionId, writer); } catch {}
